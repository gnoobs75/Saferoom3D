...
if ($gpu -match "NVIDIA") {
    ...
    Start-Process $godotPath -ArgumentList "--editor", "--path", $projectPath
} else {
    Write-Host "Using OpenGL renderer (Intel/AMD integrated detected)"
    Start-Process $godotPath -ArgumentList "--editor", "--path", $projectPath, "--rendering-driver", "opengl3"
}
